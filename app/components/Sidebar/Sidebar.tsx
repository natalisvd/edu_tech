...
async function getRole() {
  ...
  const { data: role_to_user, error, status } = await supabase
    .from('role_to_user')
    .select(`
      role (
        role_name
  ...
  const { role_name } = role_to_user?.role as RoleResponse
  if (role_name) {
    return role_name
  }
  return 'Developer'
...
export const Sidebar = async () => {
  ...
  return (
    <div className='drawer-side mt-16'>
      <label htmlFor='left-sidebar-menu' aria-label='close sidebar' className='drawer-overlay' />
